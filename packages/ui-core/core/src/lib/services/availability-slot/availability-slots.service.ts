...
import { IAvailabilitySlot, IAvailabilitySlotsCreateInput, IAvailabilitySlotsFindInput } from '@packages/contracts';
...
export class AvailabilitySlotsService {
	...
	getAll(
		...
		return firstValueFrom(
			this.http.get<{ items: IAvailabilitySlot[]; total: number }>(this.AVAILABILITY_SLOTS_BASE_URI, {
				params: { data }
			})
		);
	}
